...
test_quoting =
  "quoting"
    ~: TestList
      [ checkUnquotedVar Var "$1"
          ~= Left
            "Error: No quotes around variable"
            checkUnquotedVar
            "echo \"$1\""
          ~= Right
            Var
            ...
            "\"~/5520 file.txt\""
          ~= Left
            "Error: Misplaced quotes around tilde expression"
            checkQuotedTildeExpansion
            "~/5520 file.txt"
          ~= Right
            "rm ~/5520 file.txt"
            ...
            "\'it's not right\'"
          ~= Left
            "Error: Single quote closed by apostrophe"
            checkSingleQuoteApostrophe
            "\'it'\''s a string\'"
          ~= Right
            "\'it'\''s a string\'"
            ...
            "\'it\'s a string\'"
          ~= Left
            "Error: Can't escape single quote inside single quotes"
            checkVarInSingleQuotes
            "\'$1\'"
          ~= Left "Error: Variable can't be used in single quotes"
      ]
  ...
  "conditional"
    ~: TestList
      [ checkMissingSpaces Right Conditional "if [\"$foo\"==0]\n" ["then echo \"true\"\n", "else echo \"false\"\n", "fi"]
          ~= Left
            "Error: Missing spaces around equal-to operator"
